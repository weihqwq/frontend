...
const Deuteronomy = () => {
  ...
  //console.log({jwt});
    return (
      ...
      <button onClick={() => navigate(`/KindSelection2?jwt=${jwt}`)} style={{ width: '50%', height: '100px', fontSize: '32px', backgroundColor: 'lightgreen', marginBottom: '20px', borderRadius: '15px',color:'black '  }}>
        分类训练
      </button>
      ...
      <button onClick={() => navigate(`/PaperSelection1?jwt=${jwt}`)} style={{ width: '50%', height: '100px', fontSize: '32px', backgroundColor: 'lightblue', marginBottom: '20px', borderRadius: '15px',color:'black '  }}>
                限字训练
        </button>
    </div>
    );
